...
interface ResponseSample {
  ...
  statusCode: number;
  usesFaker: boolean; // fakerを使用するかどうか
}
...
function extractResponseSamples(mswContent: string): ResponseSample[] {
  ...
  while ((match = responseMockRegex.exec(mswContent)) !== null) {
    ...
    // faker を使っているかどうかを検知
    const usesFaker = body.includes("faker.");

    if (usesFaker) {
      console.log(
        `⚠️  ${functionName}: faker使用を検知しました。fakerをそのまま使用します`,
      );
...
function generateResponseFile(sample: ResponseSample): string {
  ...
  // fakerを使用する場合はimportを追加
  const fakerImport = sample.usesFaker
